...
object RunGcamBatch {
    ...
    def main(args: Array[String]) : Unit = {
        ...
        val spark: SparkSession = SparkSession.builder()
            .appName("RunGcamBatch")
            .master("local")
            //.master("yarn")
            //.config("spark.submit.deployMode","cluster")
            .config("spark.hadoop.fs.s3.awsAccessKeyId", "AKIAIPUDUPXBC7Z33SIA")
            .config("spark.hadoop.fs.s3.awsSecretAccessKey", "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")
            .getOrCreate()
        ...
        val inputDir = "s3://raw-gkg-gdelt/*"
        val outputDir = "s3://gdelt.storage/"

    	val GkgCoreSchema = new StructType(Array(
        ...
        val gkgDF = gkgRaw.toDF

        val gcamRaw = gkgDF.select("GkgRecordID","V21Date","V15Tone","V2GCAM", "V1Locations")
        	gcamRaw.cache()
        	gcamRaw.registerTempTable("gcamRaw")

        // wrap the imported geohash funciton to register as a udf

        spark.udf.register("geo_hash", geo_wrap(_:Double, _:Double, _:Int))

        val extract_gcam = spark.sql("""
        	SELECT 
        		GkgRecordID 
            ,   V21Date
            ...
            ,   regexp_replace(V15Tone, ",.*$", "") AS V15Tone      -- trunc off the other scores
            FROM gcamRaw
            WHERE
            	length(V2GCAM) > 1
